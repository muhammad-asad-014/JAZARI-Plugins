...
bp = Blueprint('jaz', __name__, template_folder='templates', root_path=plugin_dir, static_folder='static')
...
def home():
    # for teachers
    return render_template('jaz-home.html')
...
def settings():
    ...
    return render_template('jaz-settings.html')
...
def update_settings():
    ...
    if request.method=='POST':
        ...
        return redirect(url_for('jaz.update_settings'))
    return render_template('jaz-settings.html')
...
def ask_llm():
    ...
    try:
            ...
            messages=[
                {
                    "role": "system",
                    "content": (
            "You are JAZ (Just in time Assistance Zone), a concise teaching assistant. "
        ...
        answer = f"JAZ encountered an API error: {str(e)}"
